...
4. **서브에이전트는 완전 블랭크.** 사전 컨텍스트 없음. 스킬(SKILL.md)이 유일한
   방법론 소스, CLI(packages/cli/bin/faraday.mjs)가 유일한 도구. **품질 기준을 프롬프트에
...
- The CLI npm package is NOT published; whenever the skill says
  `npx @faraday-kit/cli@latest`, use `node {{repo}}/packages/cli/bin/faraday.mjs` instead.
...
- Do NOT edit anything under the repo itself (packages/, plugins/).
...
- [ ] **scaffold 문서 동기화**: `packages/cli/templates/starter/AGENTS.md`,
