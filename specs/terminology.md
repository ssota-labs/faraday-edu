...
| Course | `Curriculum` 타입, `<Course>`, `CurriculumHost` | Phase 4에서 타입 정리 |
| Lecture | `CurriculumNode`, `nodes/<id>.tsx`, `<Lesson>` | |
| SlideView | `deck` 팩, `<Paged>` | → `slide-view`, `<SlideDeck>` |
...
- `<Paged>` → `<SlideDeck>` (슬라이드 뷰 일반 모드 엔진)
- `deck` → `slide-view`
...
| 4 | `Curriculum*` → `Course*` 타입·컴포넌트 리네임 |
