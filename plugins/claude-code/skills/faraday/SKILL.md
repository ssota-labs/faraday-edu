...
**courses** and **worlds**. Your job in this skill is to drive that end-to-end:
scaffold → author → pass the gates → (optionally) deploy.
...
5. **Verify live:** `pnpm dev` — Vite prints a Local URL on a free port (read it
   ...
   with a `<Quiz>`.
6. **Ship (optional):** `pnpm build` → static `dist/`, or deploy (see
   `/faraday-deploy`).
...
- **2D (default)** — diagrams, charts, algorithm walk-throughs, parameter
  exploration. Stays light; no `three`. This is the right default for most topics.
...
hands-off author pass in a clean session, delegate to the **faraday-author**
subagent.
