...
- `linearPack` — status list (doc-style, renders inline). `@faraday-academy/kit/world`
- `map2dPack` — 2D tactical node map (**game screen**). `@faraday-academy/kit/world`
...
The `@faraday-academy/kit/lms` module exposes a progress recorder + dashboard components
that attach to a lesson or a whole curriculum. Wire it to the `CurriculumHost`
event stream:

```tsx
import { CurriculumHost } from "@faraday-academy/kit/world";
...
Like everything in the kit, it's a pinned dependency — compose it, don't edit it.
