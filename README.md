...
> 터미널 없이 — 코딩 에이전트 채팅에 **메시지 두 번**이면 됩니다.
...
(에이전트가 이미 있으면 끝입니다. Node/pnpm을 직접 다룰 필요는 없습니다.)
...
```text
복리를 체감할 수 있는 인터랙티브 수업을 만들어줘.
원금·이율·기간을 슬라이더로 조절하고, 근거 기반 AI 튜터도 붙여줘.
```

에이전트가 스캐폴드 → 코드 작성 → 검사 → 브라우저 미리보기까지 진행합니다.  
끝나면 채팅에서 이렇게만 이어가면 됩니다:
...
말하는 대로 **조작 가능한 수업 앱**(Vite + React)을 만들고, 원하면 **근거 기반 AI 튜터**·3D·퀴즈·커리큘럼까지 붙입니다.  
