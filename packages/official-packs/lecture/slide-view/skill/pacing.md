...
- **Length** — slide view is short by nature. Aim for the fewest slides that carry the
  arc; if it runs past ~15–20, it's probably a **textbook view** or a `<Course>` (chaptered) instead.
- **Navigation is expected** — `<SlideDeck>` ships prev/next, a dot rail, and arrow
...
- **Self-contained slides** — because per-slide state resets, a slide the reader
  jumps back to must still make sense on its own.

→ **Faraday:** `<SlideDeck>` for the slide view; a prediction `<Quiz>` slide before a reveal
