// Recursive copy with an ignore list. We roll our own (instead of fs.cp) so the
// ignore set and symlink handling are explicit and identical across platforms.
...
/** Copy `from` into `to`, creating `to`. Skips entries whose basename is in ignore. */
...
/** True when `dir` is missing, empty, or holds only inert entries (.DS_Store). */
