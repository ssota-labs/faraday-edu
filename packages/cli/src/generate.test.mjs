...
test("--tutor pins the widget + drops the author-editable server (addons=tutor)", async () => {
  ...
  // the server needs these directly (it's an author-editable template, not a package)
  for (const dep of ["ai", "@ai-sdk/workflow", "workflow", "nitro", "zod"]) {
    assert.ok(pkg.dependencies[dep], `server dep ${dep} missing`);
  }
  // the durable server lands as EDITABLE files in the lesson tree
  ...
  // Vite+Nitro config + the dev-crash workaround + the style scan are wired
