...
The runtime ships these shadcn primitives at `@faraday-academy/ui/components/ui/*` (button, card, slider,
...
> **Pack `exam`:** installed via `faraday pack add exam`. Authoring guide at `.faraday/packs/exam`. Load it when the goal is a practice test / mock exam — assessing mastery across a topic rather than teaching one concept.
...
> **Pack `sim2d`:** installed via `faraday pack add sim2d`. Authoring guide at `.faraday/packs/sim2d`. Load it when the lesson needs a formula-driven 2D simulation or explanatory SVG animation inside a Workbench (kinematics, vectors, waves) — not a Pixi game stage.
