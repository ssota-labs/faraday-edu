...
> **Pack `assets-2d`:** installed via `faraday pack add assets-2d`. Authoring guide at `.faraday/packs/assets-2d`. Load it when the lesson needs 2D sprites, tilesets, or backgrounds for a game-view scene — sourcing CC0 art or generating sprite sheets.

> **Pack `game-view`:** installed via `faraday pack add game-view`. Authoring guide at `.faraday/packs/game-view`. Load it when the lecture should play like a preschool/elementary game — walking, dialogue, scene changes — not slides or scroll.
