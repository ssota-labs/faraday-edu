...
- **Simulation hooks** (from `src/lesson/sim2d`, the `sim2d` pack) — use these instead of hand-rolled
  rAF so nothing snaps: `useSimTime` / `useSimLoop` / `useAnimatedValue` (keep dynamic systems
