...
> **이미 가르치고 있는 수업 하나를, 근거 기반 AI 튜터까지 붙은 고품질 인터랙티브 교과서로 — 명령어 한 번에.**
>
> 상태: CLI 단계 (Phase 0 완료, GTM Stage 1로 진행 중). 아키텍처는
> [VISION.md](docs/VISION.md), 고객 전략은 [GTM.md](docs/GTM.md)를 참고하세요.
...
터미널을 열 일이 전혀 없습니다. 아래 내용은 전부 여러분의 **코딩 에이전트에게
채팅으로 말하는 것**입니다 (Claude Code, Cursor, Codex) — 실제 명령은 에이전트가
대신 실행합니다. 메시지 두 개면 수업이 나옵니다.

**1. 에이전트에게 Faraday 스킬을 설치해 달라고 하세요.** 채팅에 이렇게
붙여넣으세요:

```text
인터랙티브 수업을 만들 수 있도록 Faraday 스킬을 설치해줘.
실행: npx skills add ssota-labs/faraday-academy
```

**2. 원하는 수업을 평범한 말로 요청하세요** — 외울 플래그도 없습니다:

```text
이진 탐색을 가르치는 인터랙티브 수업을 만들어줘.
근거 기반 AI 튜터도 붙여줘.
```

이게 전부입니다. 그다음부터는 스킬이 전체 루프를 대신 처리합니다 — 수업을
스캐폴드하고, 코드를 작성하고, 검사하고, 브라우저에 라이브 미리보기를 띄워 직접
만져볼 수 있게 하고, 요청하면 배포까지 합니다. 처음부터 끝까지 채팅 안에
머무르면 됩니다.

더 많은 복붙용 프롬프트(3D, 물리, 강좌)는 [이런 걸 만들 수
있어요](#이런-걸-만들-수-있어요--감을-잡는-예시)에 있습니다.

> **플러그인 설치가 익숙하신가요?** Claude Code 사용자는 대신
> `/plugin marketplace add ssota-labs/faraday-academy` 후
> `/plugin install faraday@faraday`를 실행해도 됩니다. 에이전트별 자세한 내용은
> [`plugins/claude-code/`](plugins/claude-code/)와
> [`plugins/codex/`](plugins/codex/)를 참고하세요.

---

## 대상 사용자 (현재 기준)

GTM **Stage 1**의 대상은 얼리어답터 *제작자*입니다 — 튜터, 조교, 교사, 강의
저자 중에서 **이미 코딩 에이전트를 쓰고 있는** 분들입니다. 도구 마찰이 전혀
없습니다: 에이전트는 이미 갖고 있고, Faraday는 그 에이전트가 구동하는
스캐폴더일 뿐입니다.
...
코딩 에이전트가 등장하면서 코드로 무언가를 *만드는 일*이 갑자기 쉽고 저렴해졌고,
교육 자료는 그 수혜를 가장 크게 볼 분야입니다. 그런데도 대부분은 여전히 **정적인
텍스트** — 슬라이드, PDF, 교과서 — 에 묶여 있습니다. 같은 개념도 **인터랙티브**로
가르치면(직접 조작하는 시뮬레이션, 적응형 퀴즈, 애니메이션, 실시간 그래프, 3D 씬,
옆에 붙은 AI 튜터) 더 잘 전달됩니다 — 학습자가 개념을 읽는 대신 직접 *해보기*
때문입니다.

새로운 꿈은 아닙니다. 플래시 시절의 과학 시뮬레이션이 인터랙티브 자료의 효과를 이미
증명했죠. 하지만 대중화되진 못했습니다 — **코드가 비쌌기** 때문입니다. 수업 하나마다
엔지니어가 필요했으니까요. 코딩 에이전트가 그 비용을 없앱니다: 이제 교육자가 원하는
걸 말로 설명하면 동작하는 인터랙티브가 나옵니다.

얼리어답터 교육자들은 이미 시도하고 있습니다. 문제는 **퀄리티엔 시행착오가 든다**는
것. 에이전트는 *어떤* 인터랙티브든 만들 수 있지만, 실제로 잘 가르치는 것 — 알맞은
상호작용, 알맞은 평가, 일관된 룩 — 을 얻으려면 수많은 반복이 필요합니다.

Faraday는 그 퀄리티를 **미리 앞당겨둡니다.** 좋은 교육 콘텐츠가 재사용하는
**컴포넌트와 모듈**을, 그리고 에이전트가 그것들을 잘 조합하도록 가르치는 **스킬**을
함께 제공합니다 — 그래서 여러분은 *의도*만 전달하고, 에이전트는 훨씬 적은 시행착오로
높은 퀄리티의 자료를 만듭니다.

그리고 역량을 **모듈 팩**으로 포장함으로써 — 수업의 겉면 UI부터 *가르치는 일 그
자체의 소프트웨어*(교수법·방법론)까지 — 누구나 자신의 노하우를 담아 공유할 수
있습니다. 지향점은 훌륭한 수업 포맷이나 검증된 교수법이 `faraday pack add` 한 번
거리에 있는 **커뮤니티**입니다.
