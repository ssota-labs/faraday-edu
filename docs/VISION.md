...
```
┌─ 플랫폼 (멀티테넌트, 관리형 AI, 결제)  ← mirror-dimension tier
...
| | CLI 페이즈 (지금) | 플랫폼 페이즈 |
|---|---|---|
| 배포 | 자체 호스팅 / static | **Vercel Platforms** 멀티테넌트 (텍스트북당 서브도메인) |
| 튜터 AI 키 | BYOK (제작자가 자기 키) | **우리 키** via Vercel **AI Gateway** — 사용량 미터링 |
| AI 비용 | 제작자 부담 | **튜터가 사용량만큼 지불** (Gateway 관측/과금) |
| 학생 | — | 우리 플랫폼에 로그인 |
| 결제 | — | 학생 → 튜터 (**Vercel/Stripe Connect**), 플랫폼 수수료 차감 |

**3면 마켓플레이스**: 우리(플랫폼+AI+게이트) / 튜터(제작자, AI값 지불·학생에게 수익) /
학생(학습·튜터에게 결제).
...
- **튜터 AI** (신규·가장 어려움):
  - **그라운딩/충실성**: 제작자 자료 RAG, 출처 인용, 모르면 거부(환각 금지)
  - **평가 무결성**: 정답 누출 금지, 평가 모드 잠금
  - **교수법**: 소크라테스식(답을 바로 주지 않음)
  - **안전/가드레일**
- **LMS**: 데이터 프라이버시, 성적 무결성
- **플랫폼**: 테넌트 격리, **테넌트/학생별 비용 상한**(런어웨이 AI 지출 방지), 레이트리밋
...
2. **멀티테넌트 AI 비용 폭주** — 한 학생이 비용을 태울 수 있음 → 테넌트·학생별 캡, 레이트리밋.
...
- **비용의 자연스러운 정렬**: 학생 결제(Connect) → 튜터 → 튜터의 AI 사용료(Gateway). 플랫폼은
  양쪽에서 얇게(호스팅 + AI 마진 + 결제 수수료). BYOK 옵션도 병행해 파워유저 이탈 방지.
- **오프라인/자체호스팅 트랙 유지**: 오픈소스 스킬 + CLI는 계속 무료 → 신뢰·유입. 플랫폼은
  "마찰 제거(관리형 AI·결제·멀티테넌시)"로 과금. 오픈코어 전략.
