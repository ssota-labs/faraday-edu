...
| `component-curriculum.png` | 📸 | A curriculum **world** — the 2D node map or 3D constellation with locked/unlocked nodes (`<CurriculumHost>`). | Square-ish ~800×600 for the table thumbnail. |
| `component-lecture.png` | 📸 | A **paged / slideshow** lesson — one idea per screen with prev/next + dot rail (`<Paged>`). | ~800×600. |
| `component-quiz.png` | 📸 | A check in action — a `<Quiz>` with a hint, or a `<Challenge>` mission being cleared. | ~800×600. |
| `component-lms.png` | 📸 | The **LMS dashboard** — progress across a curriculum (`runtime/lms`). | ~800×600. |
